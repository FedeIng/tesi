CREATE OR REPLACE FUNCTION guild.rental_get_two_weeks(
	)
...
RETURN QUERY (SELECT ARRAY(
	...
	FROM (SELECT gg.name as game_name,
		  ...
		  WHERE gug.restitution_ts IS NULL and lending_ts < current_date - interval '14 days'
		 ) t
) as result);
END;
$BODY$;

ALTER FUNCTION guild.rental_get_two_weeks()
    OWNER TO postgres;